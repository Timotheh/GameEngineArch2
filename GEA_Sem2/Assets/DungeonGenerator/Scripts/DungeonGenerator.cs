...
public class DungeonGenerator : MonoBehaviour {
    ...
    public int RoomRandom = 0;
    public bool randomizeRoomRandomOnStart = true;
    public bool randomRoomSize = true;
    public DRandom random;

    public bool FinshedGen = false;
    public int NoRooms = 10;    // number of rooms
    public int roomsCount;  //counts thw number of rooms spawned
	...
	void Start () {
        ...
        //instance = this;
        if (randomizeRoomRandomOnStart) {
            RoomRandom = Random.Range(0, int.MaxValue);
        }

        random = new DRandom();
        random.Init(RoomRandom);

        if (randomRoomSize) {
            NoRooms = 15 + (int)(random.value() * 50f);
        }
    ...
    private void GenerateNextRoom() {
        ...
        do {
            ...
            if(doors == 1 && possibleRooms.Count > 1) {
                ...
                float chance = 1f - Mathf.Sqrt(((float)roomsCount / (float)NoRooms)); //the closer we are to target the less of a chance of changing rooms
                ...
                if (randomValue < chance) {
                    ...
                    doors = roomToTry.GetComponent<Room>().doors.Count;
                    if (doors == 1 && possibleRooms.Count > 1) {
                        float chance2 = 1f - Mathf.Sqrt(((float)roomsCount / (float)NoRooms)); //the closer we are to target the less of a chance of changing rooms
